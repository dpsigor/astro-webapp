...
import { planets, planetGlyphUnicode } from './sweph';
...
export function chartComponent(
  ...
  if (navigator.geolocation) {
    const currentPosButton = document.createElement('button');
    currentPosButton.innerHTML = '📍';
    currentPosButton.style.margin = '0 0.5rem';
    currentPosButton.style.padding = '0 0';
    currentPosButton.style.color = '#487297';
    currentPosButton.onclick = () => {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          const { latitude, longitude } = position.coords;
          latInput.value = latitude.toFixed(2);
          lonInput.value = longitude.toFixed(2);
          latInput.onchange?.(new Event('change'));
          lonInput.onchange?.(new Event('change'));
        });
      }
    };
    lonContainer.append(currentPosButton);
  }
  const nowButton = document.createElement('button');
  nowButton.innerHTML = 'Now';
  nowButton.style.margin = '0 0.5rem';
  nowButton.style.padding = '0 0.5rem';
  nowButton.style.color = '#487297';
  nowButton.onclick = () => {
    const now = dayjs().tz(timezone);
    calendarInput.value = now.format('YYYY-MM-DD');
    timeInput.value = now.format('HH:mm');
    timeInput.onchange?.(new Event('change'));
  };

  geoContainer.append(latContainer, lonContainer, nowButton);
